...
const ViewHistory: React.FC = () => {
  ...
  // Expanded section starts with null because the first card is now static
  const [expandedSection, setExpandedSection] = useState<string | null>(null);

  const toggleSection = (id: string, index: number) => {
    // Agar pehla card hai (index 0), to toggle nahi hoga
    if (index === 0) return;
      ...
      <SafeAreaView style={styles.masterContainer} edges={['top', 'bottom']}>
        ...
        >
          {historyData.map((visit, index) => {
            // Check if card should be open (either it's the first card OR it's the expanded one)
            const isOpen = index === 0 || expandedSection === visit.id;
              ...
              <React.Fragment key={visit.id}>
                ...
                <View style={[
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => toggleSection(visit.id, index)}
                    activeOpacity={index === 0 ? 1 : 0.7}
                  >
                    ...
                    {/* Only show icon for items that can toggle (index > 0) */}
                    {index > 0 && (
                      <Ionicons
                  ...
                  {/* --- Body Section (Visible if isOpen is true) --- */}
                  {isOpen && (
